...
class TFOperatorReader(StreamReader):
    ...
    def __init__(self, config):
        start_offset = int(config[OperatorConstants.READER_START_OFFSET]) \
            if OperatorConstants.READER_START_OFFSET in config \
            else OperatorConstants.DEFAULT_QUEUE_START_OFFSET

        max_slot_size = int(config[OperatorConstants.READER_MAX_SLOT_SIZE]) \
        ...
        log_interval = int(config[OperatorConstants.READER_LOG_INTERVAL]) \
            if OperatorConstants.READER_LOG_INTERVAL in config \
            else OperatorConstants.DEFAULT_LOG_DETAIL_INTERVAL

        force_clear = config[OperatorConstants.READER_FORCE_CLEAR_WHEN_FULL_AND_ALL_CONSUMED] \
        ...
        self.__data_buffer = RingBuffer(start_offset=start_offset,
    ...
    def put_data(self, msg):
        if not self._stop_event.is_set():
            ...
            raise Exception(
                ...
                if not self._stop_reason else self._stop_reason)
    ...
    def fetch_data(self, batch_size, **options):
        ...
        while len(batch_data) < batch_size:
            ...
            batch_data.append(
                (self._consumed_offset, str(item, encoding="utf-8")))
    ...
    def iterator(self, batch_size, **options):
        ...
        while True:
            ...
            if len(batch_data) % 100 == 0:
                logger.debug("in iterator batch data size is: %d" %
                             len(batch_data))
    ...
    def seek_by_offset(self, start_offset):
        ...
        try:
            ...
            logger.warning("Seek by offset caught value error: {}, "
    ...
    def stop_reader(self, reason=None):
        ...
        try:
            ...
            logger.warning(
                "Stop reader and make exception event failed {}.".format(e))
            os._exit(-1)
    ...
    def load_checkpoint(self, checkpoint_state):
        if not self.__is_operator_consumed_offset_legal(checkpoint_state):
            logger.info(
                "operator_consumed_offset is illegal, use reader_consumed_offset: {}"
                .format(checkpoint_state["reader_consumed_offset"]))
        ...
        logger.info("Tf operator consumed_offset is: {}".format(
            self._consumed_offset))
        self.__data_buffer.load_checkpoint(checkpoint_state)
        logger.debug(
            "Tf operator finished load checkpoint, current state is: {}".
    ...
    def __is_operator_consumed_offset_legal(self, checkpoint_state):
        ...
        logger.info(
            "operator_consumed_offset is: {}".format(operator_consumed_offset))
        # if operator_consumed_offset is valid, use it as reader offset to keep
        # exactly once
        if operator_consumed_offset is not None and \
                real_head <= reader_consumed_offset <= operator_consumed_offset <= real_tail:
