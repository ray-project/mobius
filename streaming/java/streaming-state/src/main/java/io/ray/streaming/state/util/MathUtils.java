...
public class MathUtils {
   ...
   * least significant bit to the most significant bit. For example, <code>log2floor(16) =
   * 4</code>,
   * and <code>log2floor(10) = 3</code>.
   *
   ...
   *
   * <p>This method is a protection in places where one expects to be able to safely case, but where
   * unexpected
   * situations could make the cast unsafe and would cause hidden problems that are hard to track
   * down.
   *
   ...
   * The hash function has the <i>full avalanche</i> property, meaning that every bit of the
   * value to be hashed affects every bit of the hash value.
   *
