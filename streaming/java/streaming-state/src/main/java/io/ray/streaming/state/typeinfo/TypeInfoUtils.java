...
public class TypeInfoUtils {
  ...
  private static <T> TypeInformation<T> createTypeInfoWithTypeHierarchy(
    ...
    if (type instanceof ParameterizedType) {
      return (TypeInformation<T>)
          createTypeInfoForClass(typeConvertToClass(type), (ParameterizedType) type, typeHierarchy);
  ...
  private static <T> TypeInformation<T> createTypeInfoForClass(
      Class<T> clazz, ParameterizedType parameterizedType, List<Type> typeHierarchy) {
    ...
    if (clazz.isArray()) {
      ...
      TypeInformation<?> componentTypeInfo =
          createTypeInfoWithTypeHierarchy(clazz.getComponentType(), typeHierarchy);
  ...
  /**
   * Validation rules: 1. The class access permission is public. 2. The class field is public access
   * or has corresponding setter and getter 3. The class has a default no-argument constructor, and
   * the access permission is public.
   */
  private static <T> PojoTypeInfo<T> isPojoTypeClass(
      Class<T> classType, List<Type> typeHierarchy, ParameterizedType parameterizedType) {
    try {
      // check class access type
      if (!Modifier.isPublic(classType.getModifiers())) {
        LOG.info(
            "Class {} is not public so it cannot be used as a POJO type.", classType.getName());
        return null;
      }
      ...
      for (Field field : fields) {
        ...
        if (!isValidPojoField(field, classType)) {
          LOG.info(
              "Class {} field {} no valid POJO fields, so it cannot be used as a POJO type.",
              classType,
              field.getName());
          return null;
        }
        ...
        try {
          ...
          TypeInformation<?> typeInfo =
              createTypeInfoWithTypeHierarchy(fieldType, fieldTypeHierarchy);
      ...
      if (!Modifier.isPublic(defaultConstructor.getModifiers())) {
        LOG.info(
            "Class {} default constructor is not Public, so it cannot be used as a POJO type.",
            classType);
        return null;
      }
  ...
  //          List<Type> fieldTypeHierarchy = new ArrayList<>(typeHierarchy);
  //          fieldTypeHierarchy.add(fieldType);
  //          TypeInformation<?> typeInfo = createTypeInfoWithTypeHierarchy(fieldType,
  // fieldTypeHierarchy);
  ...
  //  private static <T> RowKeyTypeInfo<T> isRowKeyTypeClass(Class<T> clazzType, List<Type>
  // typeHierarchy) {
  ...
  private static boolean isValidPojoField(Field field, Class<?> clazz) {
    ...
    if (Modifier.isPublic(field.getModifiers())) {
      ...
      for (Method method : clazz.getMethods()) {
        ...
        // check for getter
        // check (have getFieldName or isFieldName) && (method parameter list is empty) && (return
        // type is same as field type)
        ...
        // check for setter
        // check (have setFieldName or isFieldName) && (method parameter list length is 1)  &&
        // (method first parameter type is same as field type) && (method return type is void type)
