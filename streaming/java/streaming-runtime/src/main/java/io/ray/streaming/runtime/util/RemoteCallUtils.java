...
public final class RemoteCallUtils {
  ...
  /**
   * Ray creates actor asynchronously, so how could we get to know whether the actor is ready or not?
   * We did this by submitting an actor task and wait util it is ready, then call ray.get to fetch
  ...
  public static <T> boolean asyncBatchExecute(
    ...
    List<CompletableFuture<Boolean>> futureResults = executionVertices.stream().map(vertex ->
        CompletableFuture.supplyAsync(() -> {
          ...
          // Note, we have no strong way to make 100% sure that these threads use the
          // creater thread's class loader. Set the class loader manully here to avoid
  ...
  public static byte[] buildBytesWorkerContext(JobWorkerContext ctx) {
    ...
    RemoteCall.ActorId workerId = RemoteCall.ActorId.newBuilder()
        .setId(ByteString.copyFrom(ctx.getWorkerActorId().getBytes())).build();

    Map<String, ActorHandle> inputQueueActor = new HashMap<>();
    ctx.getInputActors().forEach((queueName, actor) -> {
      final byte[] stateBytes = ((NativeJavaActorHandle) actor).toBytes();
      RemoteCall.ActorHandle actorHandle = RemoteCall.ActorHandle.newBuilder()
          .setFromJava(true)
    ...
    ctx.getOutputActors().forEach((queueName, actor) -> {
      final byte[] stateBytes = ((NativeJavaActorHandle) actor).toBytes();
      RemoteCall.ActorHandle actorHandle = RemoteCall.ActorHandle.newBuilder()
          .setFromJava(true)
          .setActorHandle(ByteString.copyFrom(stateBytes))
    ...
    RemoteCall.JobWorkerContext ctxPb = RemoteCall.JobWorkerContext.newBuilder()
        ...
        .setWorkerState(
            RemoteCall.ExecutionVertexState.forNumber(
                ctx.getExecutionVertex().getState().code))
