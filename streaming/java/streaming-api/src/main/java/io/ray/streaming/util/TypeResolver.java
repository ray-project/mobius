...
public class TypeResolver implements Serializable {
  ...
  /** Cache of vertexType variable/argument pairs. */
  ...
  static {
    ...
    try {
      ...
      LOGGER.warn("TypeResolver static init exception", ignore);
    }
  ...
  /**
   * Returns the raw class representing the argument for the {@code vertexType} using vertexType
   * variable information from the {@code subType}. If no arguments can be resolved then {@code
   * Unknown.class} is returned.
   *
   * @param type to resolve argument for
   * @param subType to extract vertexType variable information from
   * @return argument for {@code vertexType} else {@link Unknown}.class if no vertexType arguments
   *     are declared
  ...
  /**
   * Returns the raw class representing the argument for the {@code genericType} using vertexType
   * variable information from the {@code subType}. If {@code genericType} is an instance of class,
   * then {@code genericType} is returned. If no arguments can be resolved then {@code
   * Unknown.class} is returned.
   *
   * @param genericType to resolve argument for
   * @param subType to extract vertexType variable information from
   * @return argument for {@code genericType} else {@link Unknown}.class if no vertexType arguments
   *     are declared
   * @throws IllegalArgumentException if more or less than one argument is resolved for the {@code
   *     genericType}
   */
  public static Class<?> resolveRawArgument(Type genericType, Class<?> subType) {
    ...
    if (arguments.length != 1) {
      throw new IllegalArgumentException(
          "Expected 1 argument for generic vertexType "
              + genericType
              + " but found "
              + arguments.length);
    }
  ...
  /**
   * Returns an array of raw classes representing arguments for the {@code vertexType} using
   * vertexType variable information from the {@code subType}. Arguments for {@code vertexType} that
   ...
   * @param subType to extract vertexType variable information from
   * @return array of raw classes representing arguments for the {@code vertexType} else {@code
   *     null} if no vertexType arguments are declared
   */
  ...
  /**
   * Returns an array of raw classes representing arguments for the {@code genericType} using
   * vertexType variable information from the {@code subType}. Arguments for {@code genericType}
   * that cannot be resolved are returned as {@code Unknown.class}. If no arguments can be resolved
  ...
  /**
   * Returns the generic {@code vertexType} using vertexType variable information from the {@code
   * subType} else {@code null} if the generic vertexType cannot be resolved.
   *
   * @param type to resolve generic vertexType for
   * @param subType to extract vertexType variable information from
   * @return generic {@code vertexType} else {@code null} if it cannot be resolved
   */
  ...
  /**
   * Resolves the raw class for the {@code genericType}, using the vertexType variable information
   * from the {@code subType} else {@link Unknown} if the raw class cannot be resolved.
   *
  ...
  /** Populates the {@code map} with variable/argument pairs for the given {@code vertexType}. */
  ...
  private static void populateLambdaArgs(
    ...
    if (RESOLVES_LAMBDAS) {
      ...
      for (Method m : functionalInterface.getMethods()) {
        if (!isDefaultMethod() && !Modifier.isStatic(m.getModifiers()) && !m.isBridge()) {
          ...
          // Get functional interface's vertexType params
          ...
          // Populate return vertexType argument
          ...
          // Populate object vertexType from arbitrary object method reference
          int paramOffset = 0;
  ...
  private static boolean isDefaultMethod() {
  ...
  private static Member getMemberRef(Class<?> type) {
    ...
    for (int i = getConstantPoolSize(constantPool) - 1; i >= 0; i--) {
      ...
      // Skip SerializedLambda constructors and members of the "vertexType" class
      if (member == null
  ...
  /** An unknown vertexType. */
  public static final class Unknown implements Serializable {
